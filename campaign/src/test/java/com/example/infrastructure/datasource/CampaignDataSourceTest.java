...
class CampaignDataSourceTest {
  ...
  void findOngoingCampaigns() {
    ...
    assertTrue(result.list().stream().anyMatch(c -> "0nss8Qp2du2cSWT".equals(c.number().value())));
    assertTrue(result.list().stream().anyMatch(c -> "dOY0aXcFv3grfpT".equals(c.number().value())));
  }
  ...
  void findNonExistingCampaign() {
    ...
    ResourceNotFoundException exception =
        assertThrows(
            ResourceNotFoundException.class, () -> campaignDataSource.findCampaign(number));
